...
import { dateFormat } from '@app/core/helper/date'
...
@Component({
    ...
    apollo: {
        block: {
            ...
            variables() {
                return { blockRef: this.isHash ? this.blockRef : parseInt(this.blockRef), chain: this.network }
            },
            skip() {
...
export default class BlockDetails extends Mixins(NumberFormatMixin, NewBlockSubscription, NetworkMixin) {
    /*
    ===================================================================================
      Props
    ===================================================================================
    ...
    */
    ...
    get txFeeField(): string {
        return this.$i18n.tc('tx.fee', 2)
    }
    ...
    get blockDetails(): Detail[] {
        ...
        if (this.loading) {
            ...
            details = [
                ...
                {
                    title: this.$i18n.t('common.timestmp'),
                    detail: moment(this.block.summary.timestamp * 1e3).format(dateFormat.SORT)
                },
                {
                    title: this.$i18n.t('miner.name'),
                ...
                {
                    title: this.$i18n.t('miner.total-rewards'),
                    detail: `${this.rewards.value} ${this.isTrx ? this.$i18n.t('common.trx') : this.rewards.unit}`,
                    tooltip: `${this.rewards.tooltipText} ${this.isTrx ? this.$i18n.t('common.trx') : this.$i18n.t('common.eth')}` || undefined
                },
                {
                    title: this.txFeeField,
                    detail: `${this.transactionFees.value} ${this.isTrx ? this.$i18n.t('common.trx') : this.transactionFees.unit}`,
                    tooltip: this.transactionFees.tooltipText
                        ? `${this.transactionFees.tooltipText} ${this.isTrx ? this.$i18n.t('common.trx') : this.$i18n.t('common.eth')}`
                        : undefined
                },

                {
                    title: this.$i18n.t('uncle.reward'),
                    detail: `${this.uncleRewards.value} ${this.isTrx ? this.$i18n.t('common.trx') : this.uncleRewards.unit}`,
                    tooltip: this.uncleRewards.tooltipText
                        ? `${this.uncleRewards.tooltipText} ${this.isTrx ? this.$i18n.t('common.trx') : this.$i18n.t('common.eth')}`
                        : undefined
                },
                {
                    title: this.$i18n.tc('tx.name', 2),
                ...
                return this.isTrx ? detail.title !== this.txFeeField : detail
            })
        }
        return details
    }
