...
export default class AddressTokens extends Mixins(CoinData, NetworkMixin) {
    /*
    ===================================================================================
      Props
    ===================================================================================
    ...
    */
    ...
    get message(): string {
        if (!this.loading && this.hasTokens && this.getTokens.length === 0) {
            if (this.isNFT) {
                return `${this.isTrx ? this.$t('message.token.no-nft-addr.trx') : this.$t('message.token.no-nft-addr.eth')}`
            }
            return `${this.isTrx ? this.$t('message.token.no-tokens-addr.trc20') : this.$t('message.token.no-tokens-addr.erc20')}`
        }
        return ''
    }

    get getTitle(): string {
        return this.isERC20 ? `${this.isTrx ? this.$t('token.trc20') : this.$t('token.erc20')}` : `${this.$t('token.nft')}`
    }
