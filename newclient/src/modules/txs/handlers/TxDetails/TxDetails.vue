...
@Component({
    ...
    apollo: {
        transaction: {
            ...
            variables() {
                return { hash: this.txRef, chain: this.network }
            },
...
export default class TxDetails extends Mixins(NumberFormatMixin, NetworkMixin) {
    /*
  ===================================================================================
    Props
  ===================================================================================
    ...
    get txDetails(): Detail[] {
        ...
        if (this.isLoading) {
            ...
            details = [
                ...
                {
                    title: this.$i18n.t('common.amount'),
                    detail: `${this.txAmount.value} ${this.isTrx ? this.$t('common.trx') : this.$t(`common.${this.txAmount.unit}`)}`,
                    tooltip: this.txAmount.tooltipText
                        ? `${this.txAmount.tooltipText} ${this.isTrx ? this.$t('common.trx') : this.$i18n.t('common.eth')}`
                        : undefined
                },

                {
                    title: this.$i18n.tc(this.pendingString, 1),
                    detail: `${this.txFee.value} ${this.isTrx ? this.$t('common.trx') : this.$t(`common.${this.txFee.unit}`)}`,
                    tooltip: this.txFee.tooltipText ? `${this.txFee.tooltipText} ${this.isTrx ? this.$t('common.trx') : this.$i18n.t('common.eth')}` : undefined
                },
                {
                ...
                {
                    title: this.$i18n.t('gas.price'),
                    detail: `${this.gasPrice.value} ${this.isTrx ? this.$t('common.trx') : this.$t(`common.${this.gasPrice.unit}`)}`,
                    tooltip: this.gasPrice.tooltipText
                        ? `${
                              this.gasPrice.tooltipText.includes('trx')
                                  ? this.gasPrice.tooltipText.replace('eth', 'trx')
                                  : this.gasPrice.tooltipText.replace('gwei', 'trx')
                          }`
                        : undefined
                },
                {
            ...
            if (this.txStatus !== 'pending' && !this.isReplaced) {
                const time = {
                    title: this.$i18n.t('common.timestmp'),
                    detail: this.transaction.timestamp !== null ? moment(this.transaction.timestamp * 1e3).format(dateFormat.SORT) : ''
                }
