<template>
    <v-dialog v-if="greet" v-model="dialog" width="500">
        <v-card>
            ...
            <v-card-text>
                {{ isTrx ? $t('message.welcome.text1TRX') : $t('message.welcome.text1ETH') }} <a :href="`mailto:${supportEmail}`">{{ supportEmail }}</a>
                {{ $t('message.welcome.text2') }}
...
import { NetworkMixin } from '../mixins/network.mixin'
@Component
export default class AppGreeting extends Mixins(GreetMixin, NetworkMixin) {
    /*
  ===================================================================================
    Initial Data
  ===================================================================================
